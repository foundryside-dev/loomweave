...
**Workstream:** WS5 of the Clarion first-class program
(`2026-06-02-clarion-first-class-program-design.md` §2). Parallel band — autonomous, ungated.
...
**Inputs / authorities:**
- `docs/clarion/1.0/system-design.md` §8 (MCP Consult Surface) — the *intended* catalogue; its
  cursor/session model is **ratified-away here** (see §1).
- `docs/clarion/1.0/requirements.md` REQ-MCP-*, NFR-PERF-02/03.
- `docs/clarion/adr/ADR-030-on-demand-summary-scope.md` — the on-demand posture this inherits.
- `docs/clarion/adr/ADR-038-sei-token-and-signature.md` — SEI carried in responses.
- Ground truth: `crates/clarion-mcp/` (the live 19-tool stateless surface).
...
and a scope-lens. **That model was never built.** Ground truth (`crates/clarion-mcp/src/`): the
...
| `findings_for`, `wardline_for` | the `clarion guidance` CLI |
...
(`docs/superpowers/plans/2026-06-02-clarion-ws5b-advanced-queries-plan.md`) and sequences as the
...
- The `clarion-workflow` skill / MCP tool docs are updated to describe the new tools (stateless,
