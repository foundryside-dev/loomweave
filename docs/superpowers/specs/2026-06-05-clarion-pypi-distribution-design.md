# Clarion PyPI Distribution — Design
...
Ship Clarion through **PyPI as the canonical install channel**, so a developer
...
```bash
uvx clarion install --path .      # or: pipx install clarion && clarion install --path .
```

installs the `clarion` binary, brings the Python plugin into the same
environment, and a subsequent `clarion analyze` **discovers and spawns the
...
## 2. Current state (v1.3.0)

- **Rust core** → `clarion` binary (`crates/clarion-cli`, a plain `bin` crate,
...
- **Python plugin** → `clarion-plugin-python` (`plugins/python`, hatchling),
  ...
  *separate spawned process* (ADR-021 plugin jail), ships `plugin.toml` as
  hatchling **shared-data** into `share/clarion/plugins/python/`, exposes a
  `clarion-plugin-python` console script, and depends on `pyright==1.1.409`
...
`crates/clarion-core/src/plugin/discovery.rs` (the only production discovery
path, called from `crates/clarion-cli/src/analyze.rs:432`) is **`$PATH`-only**:

1. Scan each `$PATH` directory for executables named `clarion-plugin-<suffix>`
   (refusing world-writable dirs per ADR-021).
2. For each, resolve the manifest via: **neighbor** `<dir>/plugin.toml` →
   **install-prefix** `<dir>/../share/clarion/plugins/<suffix>/plugin.toml`
...
There is **no** `current_exe()`-relative probe, no `CLARION_PLUGIN_PATH`, no
explicit registry. Every test exercises discovery by setting
`.env("PATH", plugin_path)`.

**Consequence:** `pipx install clarion` / `uvx clarion` expose **only** the
`clarion` entry point on the user's `$PATH`. The dependency's
`clarion-plugin-python` script lands in the venv `bin/` but is *not* on `$PATH`
...
| D2 | **Approach A: two PyPI projects**, `clarion` depends on `clarion-plugin-python` | Keeps the ADR-021 core/plugin boundary; one-command UX via dependency resolution; both already version in lockstep |
...
| D4 | **Rust binary shipped via maturin `bindings = "bin"` platform wheels** | The ruff/uv pattern; `clarion-cli` is already a plain bin crate |
...
1. **`clarion`** — platform wheels built by maturin (`bindings = "bin"`), each
   carrying the cross-compiled `clarion` binary placed as a wheel script (installs
   into `<venv>/bin/clarion`). Declares a pinned runtime dependency
   `clarion-plugin-python == <workspace version>`.
2. **`clarion-plugin-python`** — the existing pure-Python package, now built as a
   **wheel** in addition to the sdist. Unchanged shape: ships `plugin.toml` shared-data
   into `<venv>/share/clarion/plugins/python/`, exposes the `clarion-plugin-python`
   console script (installs into `<venv>/bin/`), depends on `pyright`/`pyyaml`/`packaging`.

**Resulting venv layout** (what `uvx clarion` / `pipx install clarion` produces):

```
<venv>/bin/clarion                                  # maturin bin wheel
<venv>/bin/clarion-plugin-python                    # plugin console script
<venv>/share/clarion/plugins/python/plugin.toml     # plugin shared-data
```

`clarion` and the plugin are co-located in the same venv `bin/` and `share/`.
...
scan the directory containing the **running `clarion` binary**
(`std::env::current_exe()?.parent()`) for `clarion-plugin-*` executables, feeding
each through the *existing* `load_plugin` / `find_manifest` logic (which already
handles the `bin/ → ../share/clarion/plugins/<suffix>/` install-prefix layout).

- This makes the co-located plugin discoverable with **no `$PATH` manipulation**:
  `current_exe()` is `<venv>/bin/clarion`, its parent is `<venv>/bin`, and the
  existing install-prefix probe resolves `<venv>/share/clarion/plugins/python/plugin.toml`.
...
- **Security:** the new directory is subject to the **same world-writable
  ...
  running `clarion` binary is implicitly as trusted as that binary — an attacker
  who can write a sibling `clarion-plugin-*` there can already replace `clarion`
...
1. `uvx clarion install --path .` → uv resolves the `clarion` platform wheel +
   `clarion-plugin-python` wheel into one environment; `clarion install` does its
   existing setup (`.clarion/`, skills, MCP config, hook).
2. `clarion analyze` → `discover()` scans `$PATH` **and** `current_exe()` dir →
   finds `<venv>/bin/clarion-plugin-python` → resolves
   `<venv>/share/clarion/plugins/python/plugin.toml` → host spawns the plugin
...
- **New wheels job(s)** (in `release.yml` or a dedicated `wheels.yml`), gated
  ...
  - `clarion` platform wheels via maturin (`bindings = "bin"`) for the standard-4
  ...
  - `clarion-plugin-python` wheel via `uv build` (pure Python, one wheel).
  - **sdist for `clarion`**: produced, but gated by a **clean-room build test**
    ...
    fall back to **no `clarion` sdist** and route off-matrix platforms to the
...
- **Release ordering:** publish `clarion-plugin-python` **before** `clarion`, so
  the `clarion` wheel's `== <version>` dependency resolves on the first release.
...
- **Version lockstep:** extend `check-workspace-version-lockstep.py` to assert the
  two PyPI package versions **and** the `clarion → clarion-plugin-python` pin all
...
- First `clarion analyze` needs network to fetch Node (surprising; breaks
  air-gapped/offline installs).
- A runtime network fetch sits awkwardly with Clarion's untrusted-corpus posture.

**v1 plan:** document the first-run fetch in the README install section, and
prime the pyright/Node cache during `clarion install` (a cache-warm step) so the
...
- **Discovery spike → permanent test (no PyPI needed):** an integration test that
  stages a `<tmp>/bin/clarion` (the built binary) + `<tmp>/share/clarion/plugins/python/plugin.toml`
  + `<tmp>/bin/clarion-plugin-python`, runs `clarion analyze` with the plugin dir
...
- **Clean-room sdist build test** (gates D5's sdist decision): in a minimal
  container with only a Rust toolchain, `pip install clarion-<ver>.tar.gz` and run
  `clarion --version`.
- **Per-platform install-and-spawn smoke test:** on each matrix OS, install the
  built wheels from a local index / **TestPyPI**, run `clarion --version`,
  `clarion install --path <tmp>`, and a tiny `analyze` proving plugin discovery in
...
- README install section collapses from the 3-step tarball dance to
  `uvx clarion install --path .` (and the `pipx` equivalent), with the signed-tarball
...
| `clarion` sdist (workspace path-deps + bundled C) won't compile on a user box | Medium | Clean-room sdist build test; fall back to no-sdist + GitHub Release/cargo-binstall route |
...
| Node fetch breaks offline/air-gapped first run | Medium | Document + cache-warm in `clarion install`; hermetic-Node follow-up |
| First-release dependency resolution (`clarion` needs plugin already on PyPI) | Low | Publish plugin before core |
...
- A third discovery source: the directory of the running `clarion` binary
...
- The trust rationale: co-location with the trusted `clarion` binary; an attacker
  who can write there can already replace `clarion`.
