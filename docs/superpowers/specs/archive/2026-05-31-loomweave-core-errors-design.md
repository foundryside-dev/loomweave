# Design — `loomweave-core::errors` shared error vocabulary
...
Loomweave emits structured error codes on two independent wire surfaces, each with
its own hand-maintained, drift-prone taxonomy:

1. **HTTP federation read API** (`crates/loomweave-cli/src/http_read.rs`) — a private
...
2. **MCP tool-error envelope** (`crates/loomweave-mcp/src/lib.rs`) — **bare kebab-case
...
**Co-locate, keep per-surface wire spellings.** A single `loomweave-core::errors`
...
### 3.1 New module `crates/loomweave-core/src/errors.rs`

`loomweave-core` already depends on `serde` and `thiserror`; no new dependencies.
...
- `crates/loomweave-core/src/lib.rs`: add `pub mod errors;` and, per the
  clarion-29acbcd042 facade policy, re-export at the crate root:
  `pub use errors::{HttpErrorCode, McpErrorCode};`.

- `crates/loomweave-cli/src/http_read.rs`: delete the local `enum ErrorCode` and its
  derives; add `use loomweave_core::HttpErrorCode as ErrorCode;`. All 44 existing
  ...
  loomweave-cli — they bind `StorageError` (loomweave-storage) and `StatusCode`
  (axum/http), neither of which loomweave-core sees. No wire change.
...
- `crates/loomweave-mcp/src/lib.rs`:
  ...
  - `storage_retryable(&StorageError)` stays in loomweave-mcp unchanged (it depends on
...
- **ADR-037** (new, short) — records the decision: co-locate two typed error
  vocabularies in `loomweave-core::errors`; do not merge; keep per-surface wire
...
- **`docs/federation/contracts.md`** — wire is unchanged; add an additive pointer
  noting the HTTP `code` enum is now defined canonically in `loomweave_core::errors`.
- The arch-analysis snapshots (`docs/arch-analysis-*`) are historical and are **not**
  edited.

## 6. Acceptance

- `McpErrorCode` and `HttpErrorCode` both live in `loomweave-core::errors`; both
