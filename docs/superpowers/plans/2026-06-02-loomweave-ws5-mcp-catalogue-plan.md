...
**Date:** 2026-06-02
**Design (source of truth):** `docs/superpowers/specs/2026-06-02-loomweave-ws5-mcp-catalogue-design.md`
...
- **Tool registry** lives in `crates/loomweave-mcp/src/lib.rs`: `list_tools()` (Vec<ToolDefinition>)
...
- **emit_observation:** no HTTP observation-write route exists (Filigree 2.2.0: `GET
  /api/weft/observations` 200, `POST` 405); ADR-016's sanctioned `filigree mcp` subprocess
...
New submodule `crates/loomweave-mcp/src/catalogue/` (child of crate root → can access `ServerState`
...
- [ ] **Task 6 — Docs.** Update `crates/loomweave-mcp/assets/skills/loomweave-workflow/SKILL.md` to
