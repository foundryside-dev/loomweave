...
**Workstream:** WS7 of the Clarion first-class program — **Wave 7**. Code-intelligence half;
ungated/concurrent.
...
**Inputs / authorities:**
- `docs/clarion/1.0/system-design.md` §2 (Core / Plugin Architecture — the protocol this publishes)
- `docs/clarion/adr/ADR-022-core-plugin-ontology.md` (core stays language-agnostic; plugin owns
...
- Ground truth: the protocol IS implemented (`clarion-core/src/plugin/{manifest,host,discovery}.rs`)
  and `clarion-plugin-fixture` exists — but there is **NO external protocol documentation** and no
...
| **2. Conformance harness** | generalise `clarion-plugin-fixture` into a suite a new author runs | none |
...
**1.2 Conformance harness — the plugin oracle.** Generalise `clarion-plugin-fixture` into a
...
language-appropriate equivalent) → `~/.config/clarion/plugins.toml` registration → discovery →
...
- [ ] **T2 — conformance harness.** Generalise `clarion-plugin-fixture` into a runnable conformance
...
- [ ] **T5 — docs.** Author-facing "write a Clarion plugin" guide referencing the protocol spec +
  conformance harness; `clarion-workflow`/README updates.
