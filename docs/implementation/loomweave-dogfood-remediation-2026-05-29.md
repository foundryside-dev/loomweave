...
a test-harness artifact rather than a Loomweave defect.
...
- **ARTIFACT** — caused by the eval staging, not Loomweave; re-test, don't fix.
...
- **Why the agent missed it:** the live MCP server was the pre-`5d4aeaa` binary, so
  ...
  exists to cure. **Action:** rebuild `target/release/loomweave`, restart MCP,
...
- **Fix:** (a) on `llm-invalid-json`, fall back to a deterministic **structural**
  ...
  clarion-bacd53a2ad (preview), loomweave-bacd…/spend controls are out of the epic's
...
- **Tickets touching the area:** clarion-9392f74881 (call_sites evidence),
  clarion-893c46cc5f (loomweave://context degraded-snapshot signal). Neither adds a
...
- **Eval saw:** every call errored — input path "escapes project root
  `/tmp/loomweave-b8-elspeth-full-…`" while the DB stored
  `/home/john/loomweave/tests/perf/.../core.py`. That `/tmp` root was the eval's dead
...
- **Eval saw** near-random clusters mixing Loomweave's own source with the
  `elspeth_mini` fixture — because the served DB was Loomweave's 1872-entity
...
- Not a Loomweave bug. The real elspeth analysis is correctly scoped (verified:
  36,680 elspeth entities, 0 fixture/loomweave rows). The wrong DB was *served*, via
