...
reaching the LLM provider must first pass a Loomweave-owned scanner. ADR-013
...
- New workspace crate `crates/loomweave-scanner/` implementing the ADR-013
  rule set as Rust-native regex + Shannon entropy detection.
- Baseline parser for `.loomweave/secrets-baseline.yaml` (`detect-secrets`
  v1.x format).
- CLI wiring: pre-ingest pass in `loomweave-cli::analyze::run` between
...
- MCP-side awareness of `briefing_blocked: secret_present` in
  `crates/loomweave-mcp/`'s `summary` tool dispatch.
- Five new `LMWV-SEC-*` / `LMWV-INFRA-SECRET-*` rule-IDs in the
...
- Windows support — `setrlimit` is Unix-only (`loomweave-core::plugin::limits`);
...
- Workspace at `Cargo.toml` (resolver 3, Rust 2024, MSRV 1.88). Current
  members: `loomweave-core`, `loomweave-storage`, `loomweave-cli`, `loomweave-mcp`,
  `loomweave-plugin-fixture`. The scanner becomes the **sixth**.
- `crates/loomweave-cli/src/analyze.rs` already contains:
...
- `crates/loomweave-core/src/plugin/host.rs`:
...
- Writer-actor command set in `crates/loomweave-storage/` — the scanner
...
- **`HostFinding`** (`crates/loomweave-core/src/plugin/host_findings.rs:84`) is
  ...
  the `findings` table.
- **`FindingRecord`** (`crates/loomweave-storage/src/commands.rs:113`) is the
...
`HostFinding` is **not** the channel for `LMWV-SEC-*` / `LMWV-INFRA-SECRET-*`
...
| `rule_id` | `"LMWV-SEC-SECRET-DETECTED"` (or other; see §6 catalogue) |
| `kind` | `"defect"` (per ADR-004; secrets are defects pending remediation) |
| `severity` | `"ERROR"` (or `"INFO"` for `LMWV-INFRA-SECRET-BASELINE-MATCH`) |
...
**Owner**: `crates/loomweave-scanner/`
...
| Action | Path |
|---|---|
| create | `crates/loomweave-scanner/Cargo.toml` |
| create | `crates/loomweave-scanner/src/lib.rs` |
| create | `crates/loomweave-scanner/src/patterns.rs` |
| create | `crates/loomweave-scanner/src/entropy.rs` |
| create | `crates/loomweave-scanner/tests/fixtures/*.txt` (positive + negative per rule) |
| create | `crates/loomweave-scanner/tests/scanner.rs` |
| modify | `Cargo.toml` (workspace root) — add `crates/loomweave-scanner` member |
...
Verification at exit: `cargo tree -p loomweave-scanner | head -30` shows no
`tokio` or `rusqlite` ancestors. If a downstream crate's `regex` features
leak something heavier, pin the feature set in `loomweave-scanner`'s
`Cargo.toml`.

#### Tests

`crates/loomweave-scanner/tests/scanner.rs`:
...
- `cargo test -p loomweave-scanner` green.
- `cargo clippy -p loomweave-scanner -- -D warnings` clean.
- `cargo tree -p loomweave-scanner` shows no `tokio` / `rusqlite` /
  `serde_norway` deps.
...
| create | `crates/loomweave-scanner/tests/fixtures/baselines/*.yaml` |
| modify | `crates/loomweave-scanner/tests/scanner.rs` OR new `tests/baseline.rs` |
...
`justification` missing → emit `LMWV-INFRA-SECRET-BASELINE-NO-JUSTIFICATION`
...
partitions are retained so the CLI can emit one `LMWV-SEC-SECRET-DETECTED`
per unsuppressed detection AND one `LMWV-INFRA-SECRET-BASELINE-MATCH`
...
- `cargo test -p loomweave-scanner` (combined with Task 1) green.
...
| modify | `crates/loomweave-core/src/plugin/host.rs` (stamp `briefing_blocked` into RawEntity.extra for blocked files — see "Host integration" below) |
| create | `crates/loomweave-cli/tests/secret_scan.rs` (integration tests) |
| create | `crates/loomweave-cli/tests/fixtures/secret-project/...` (fixture trees) |
...
1. Load `.loomweave/secrets-baseline.yaml` via Task 2's `load_baseline`.
...
4. Emit findings via `WriterCmd::InsertFinding` (see §3.1):
   - One `LMWV-SEC-SECRET-DETECTED` per `allowed` detection (severity
     `ERROR`).
   - One `LMWV-INFRA-SECRET-BASELINE-MATCH` per `fired_entries` element
     (severity `INFO`).
   - One `LMWV-INFRA-SECRET-BASELINE-NO-JUSTIFICATION` per
...
The plugin host (`crates/loomweave-core/src/plugin/host.rs`) currently
...
`crates/loomweave-cli/tests/secret_scan.rs`:

1. **Happy path — clean project**: fixture with no secrets → `analyze`
   exits 0; no `LMWV-SEC-*` findings persisted.
2. **One file with AWS key in `.env`**: fixture with a committed `.env`
   ...
   - Those entities' `properties_json` contains
     `"briefing_blocked":"secret_present"`.
   - `findings` table has one `LMWV-SEC-SECRET-DETECTED` row referencing
     the file.
3. **Baseline suppression**: same `.env` fixture + a `.loomweave/secrets-baseline.yaml`
   with the matching entry → no `LMWV-SEC-SECRET-DETECTED` finding; one
   `LMWV-INFRA-SECRET-BASELINE-MATCH` info-level finding; no
   `briefing_blocked` flag on entities.
4. **Baseline missing justification**: malformed baseline →
   `LMWV-INFRA-SECRET-BASELINE-NO-JUSTIFICATION` finding; analyze
...
| modify | `crates/loomweave-cli/src/secret_scan.rs` (override logic) |
| modify | `crates/loomweave-cli/tests/secret_scan.rs` (override tests) |
...
- **Detections present, no override flag**: existing block behaviour
  (Task 3) — `briefing_blocked` stamped, `LMWV-SEC-SECRET-DETECTED`
...
- **Detections present, `--allow-unredacted-secrets` only, TTY**:
  ...
  - Match → proceed without blocking; emit
    `LMWV-SEC-UNREDACTED-SECRETS-ALLOWED` per affected file; record
...
- **Detections present, `--allow-unredacted-secrets` only, non-TTY**:
  ...
  `LMWV-INFRA-SECRET-OVERRIDE-UNCONFIRMED` rule-ID on stderr (the rule-ID
...
- **Detections present, both flags, non-TTY, `--confirm-allow-unredacted-secrets=yes-i-understand`**:
  proceed; emit `LMWV-SEC-UNREDACTED-SECRETS-ALLOWED` findings; record
...
1. **Non-TTY override-confirmed**: fixture with one AWS key, both flags
   ...
   `LMWV-SEC-UNREDACTED-SECRETS-ALLOWED` per file, `runs.stats` carries
   the override keys.
2. **Non-TTY override-unconfirmed**: only `--allow-unredacted-secrets`,
   no confirm → exit 78, stderr contains
   `LMWV-INFRA-SECRET-OVERRIDE-UNCONFIRMED`, no `runs` row in the DB.
...
| Action | Path |
|---|---|
| modify | `crates/loomweave-mcp/src/lib.rs` (summary tool dispatch) |
| modify | `crates/loomweave-mcp/tests/storage_tools.rs` (or split into a new test file) |
...
```json
{
  ...
  "summary": null,
  "briefing_blocked": "secret_present",
  "remediation": "File flagged by pre-ingest secret scan. Fix the secret or whitelist via .loomweave/secrets-baseline.yaml. See ADR-013."
}
```

This is parallel to the existing `summary_scope_deferred` envelope
(`crates/loomweave-mcp/src/lib.rs:2341`) and the four `issues_unavailable`
...
|---|---|---|---|---|---|
| `LMWV-SEC-SECRET-DETECTED` | error | security | Pre-ingest secret scanner detected a credential pattern in a file slated for LLM dispatch. | Remove the secret, rotate the credential, or whitelist via `.loomweave/secrets-baseline.yaml` with a justification. | [ADR-013](../../loomweave/adr/ADR-013-pre-ingest-secret-scanner.md) |
| `LMWV-SEC-UNREDACTED-SECRETS-ALLOWED` | error | security | Operator invoked `--allow-unredacted-secrets`; file content reached the LLM provider with secrets intact. | Audit override usage via `filigree list --rule-id=LMWV-SEC-UNREDACTED-SECRETS-ALLOWED --since 30d`. | [ADR-013](../../loomweave/adr/ADR-013-pre-ingest-secret-scanner.md) |
...
| `LMWV-INFRA-SECRET-BASELINE-MATCH` | info | infra | Baseline entry suppressed a scanner detection (audit surface). | None — informational, retained for `NFR-SEC-04` audit. | [ADR-013](../../loomweave/adr/ADR-013-pre-ingest-secret-scanner.md) |
| `LMWV-INFRA-SECRET-OVERRIDE-UNCONFIRMED` | error | infra | `--allow-unredacted-secrets` supplied without confirmation; run aborted before start. | Supply `--confirm-allow-unredacted-secrets=yes-i-understand` in non-TTY contexts or run interactively. | [ADR-013](../../loomweave/adr/ADR-013-pre-ingest-secret-scanner.md) |
...
3. **How to whitelist a false positive** (edit
   `.loomweave/secrets-baseline.yaml`; example entry; commit it).
...
6. **Finding the audit trail** (
   `select * from findings where rule_id like 'LMWV-SEC-%'`;
...
|---|---|---|
| Unit | Scanner pattern fixtures (positive + negative per rule); entropy bounds; baseline parse / suppress / round-trip | `crates/loomweave-scanner/tests/` |
| Unit | Override CLI exit codes; `runs.stats` JSON shape | `crates/loomweave-cli/tests/secret_scan.rs` |
| Unit | MCP `summary` envelope on blocked entity | `crates/loomweave-mcp/tests/storage_tools.rs` |
| Integration | `analyze` over fixture project with one secret-bearing file → entities + findings + properties_json shape | `crates/loomweave-cli/tests/secret_scan.rs` |
| Integration | Baseline suppression; baseline missing-justification | same |
| Integration | Non-TTY override paths (3 cases) | same |
| E2E | `loomweave install && loomweave analyze` against a fixture project with one known secret; assert exit 0, entities present, briefing_blocked flagged, finding persisted; assert walking-skeleton fixture still green | `tests/e2e/wp5_secret_scan.sh` (new), parallels `tests/e2e/sprint_1_walking_skeleton.sh` |
...
| R-4 | Plugin-host integration to stamp `briefing_blocked` adds a code path through a 3 126-LOC file flagged by arch-analysis §5.4 A-3 | Keep the host-side change small (≤ 30 LOC, a single map lookup); the orchestration lives in `loomweave-cli/src/secret_scan.rs`, not in the host |
| R-5 | Baseline format compatibility with `detect-secrets` v1.x must be exact; an operator running `detect-secrets scan --baseline` and dropping the file in `.loomweave/` must work | Round-trip test in Task 2; smoke-test the workflow with a real `detect-secrets`-generated baseline in WS-D |
...
# Task 2
filigree create --type=task \
  --title="WP5 Task 2 — Baseline parser (.loomweave/secrets-baseline.yaml)" \
  --labels="release:v0.1,sprint:3,wp:5,adr:013,crate:scanner" \
  --priority=1
# capture as $T2
...
- [ADR-013 — Pre-Ingest Secret Scanner with LLM-Dispatch Block](../../loomweave/adr/ADR-013-pre-ingest-secret-scanner.md) — canonical spec.
- [ADR-007 — Summary cache key](../../loomweave/adr/ADR-007-summary-cache-key.md) — `briefing_blocked` interaction.
- [ADR-017 — Severity and dedup](../../loomweave/adr/ADR-017-severity-and-dedup.md) — `LMWV-SEC-*` namespace ownership.
