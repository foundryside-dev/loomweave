# Loomweave v0.1 — Implementation Plan (High-Level Overview)

**Status**: AMENDED — Sprint 1 closed (`v0.1-sprint-1`); Sprint 2 resequenced under [`sprint-2/scope-amendment-2026-05.md`](./sprint-2/scope-amendment-2026-05.md) and closed (`v0.1-sprint-2`); Sprint 3 (Weft federation hardening, ADR-014 → ADR-034) closed 2026-05-19 — see [`sprint-3/2026-05-19-weft-federation-hardening-tasking.md`](./sprint-3/2026-05-19-weft-federation-hardening-tasking.md). The WP body below is the original framing and remains the architectural reference; the Sprint 2 resequence memo and Sprint 3 tasking doc are the authoritative sources for what shipped in v1.0 vs. what slips to v1.1+. **Read all three.**
**Date opened**: 2026-04-18
**Last amended**: 2026-05-19
**Plan owner**: John Morrissey (primary dev of Loomweave, Filigree, Wardline)
**Predecessor**: [`../loomweave/v0.1/plans/v0.1-scope-commitments.md`](./v0.1-scope-plans/v0.1-scope-commitments.md) (scope locked 2026-04-18)
**Amendments**:
- [`sprint-2/scope-amendment-2026-05.md`](./sprint-2/scope-amendment-2026-05.md) (2026-05-16 — narrows WP6, splits WP9 into A/B, defers WP4/5/7/10/11-as-scheduled, pulls WP8 + WP9-A into Sprint 2 for MVP MCP surface).
- [`sprint-3/2026-05-19-weft-federation-hardening-tasking.md`](./sprint-3/2026-05-19-weft-federation-hardening-tasking.md) (2026-05-19 — Weft federation hardening sprint: bearer auth, batch resolution, `BRIEFING_BLOCKED` propagation, per-project `instance_id`; pinned in ADR-034). Closed alongside the v1.0 release.
...
flowchart LR
    ...
    WP9["WP9<br/>Weft integrations<br/>(Filigree + Wardline)"]
...
| WP9 | Weft integrations | sys §9 + sys §11 + det §7 | ADR-004, ADR-015, ADR-016, ADR-017, ADR-018 | ADR-020 |
...
summary-cache contract in ADR-007 require that, given identical inputs, Loomweave produces
...
- `AnthropicProvider` — real network calls, used by `loomweave analyze` against real repos.
...
- `loomweave install | analyze | serve` CLI entry points (skeletons; only `install`
...
- Writer-actor implementation per ADR-011: single writable connection, mpsc command
  ...
  opt-in.
- Reader-pool with connection-pool management (pool size from `loomweave.yaml`).
...
- `loomweave install` initialises `.loomweave/` correctly: empty `loomweave.db` at the right
  schema version, default `loomweave.yaml`, `.loomweave/` registered for git tracking with
...
- A blank `loomweave install` followed by `git add .loomweave && git status` shows the
...
**ADR triggers**:
- **ADR-005** (`.loomweave/` git-committable, backlog) — WP1 will force a decision on
  ...
  when implementing `loomweave install`.
...
> **Sprint 1 delivery (narrow walking-skeleton scope)** — closed 2026-04-28. The Sprint 1 slice covers L7 (Python qualname production format) and L8 (Wardline REGISTRY probe + version-pin protocol) per [`sprint-1/wp3-python-plugin.md`](./sprint-1/wp3-python-plugin.md). Function entities only (module-level + class methods); classes / decorators / module entities, edge emission (`imports`, `calls`, `decorates`, `contains`), import resolution, and the full `LMWV-PY-*` rule catalogue are deferred to the WP3-feature-complete sprint. [`sprint-1/signoffs.md §A.3`](./sprint-1/signoffs.md#a3-python-plugin-wp3) is the gate that closed.
...
qualnames Loomweave translates).

**Scope**:
- Standalone Python package `loomweave-plugin-python` (separate `pyproject.toml`).
- `plugin.toml` declaring the Python ontology: entity kinds (module, class, function,
  decorator, etc.), edge kinds (imports, calls, decorates, etc.), rule-IDs (`LMWV-PY-*`).
...
- Identity reconciliation hooks per ADR-018: produce qualnames in a
  Wardline-translatable form, with explicit translation between Loomweave's dotted
  module prefix and Wardline's separate file-path plus bare-qualname fields.
...
**Deliverables**:
- Installable Python plugin (e.g., `pipx install loomweave-plugin-python` once published;
...
- Pipeline orchestrator hosting the seven phases. WP4 implements:
  ...
  - **Phase 7 — cross-cutting structural findings**: rule evaluator producing the
    `LMWV-*` finding catalogue from `detailed-design.md` §5; severity per ADR-017.
...
**Deliverables**:
- `loomweave analyze --no-llm` runs end-to-end on a small Python repo, producing a
  populated DB, `catalog.json`, per-subsystem markdown, and `findings.jsonl`.
- Test fixtures covering each `LMWV-*` rule with positive and negative cases.
- Determinism test: running `loomweave analyze --no-llm` twice on identical input
...
- `loomweave analyze --no-llm` against the Loomweave docs tree (a real Python-light input)
...
- Baseline YAML at `.loomweave/secrets-baseline.yaml` for accepted exceptions.
...
- `--allow-unredacted-secrets` flag with explicit operator gating; emits a
  `LMWV-SEC-OVERRIDE` finding to make the override visible.
...
**Deliverables**:
- Scanner runs as the first step of `loomweave analyze`, before plugin spawn.
...
- Negative test: a file containing a known live-style secret pattern is excluded; the
  briefing for the containing subsystem omits the file; a `LMWV-SEC-FILE-EXCLUDED`
  finding is emitted.
...
**Scope**:
- `loomweave.yaml` config loader and merge semantics (project root + user home + CLI
...
- All phases gated behind RecordingProvider replay in CI; live-Anthropic runs require
  an explicit `LOOMWEAVE_LLM_LIVE=1` env var.
...
**Deliverables**:
- `loomweave analyze` (without `--no-llm`) completes end-to-end on a small fixture using
...
- Should land before WP9 (Weft integrations consume summaries via briefings).
...
- Cache-hit-rate assertion on a two-run sequence (cold then warm) ≥ a configurable
  threshold (initial threshold in `loomweave.yaml`; the actual NFR-COST-02 ≥95% number
...
- Findings:
  - `LMWV-FACT-GUIDANCE-CHURN-STALE` — guidance sheet anchored to an entity whose
    fingerprint changed since the sheet was last edited.
  - `LMWV-FACT-GUIDANCE-ORPHAN` — guidance sheet whose target entity no longer exists.
...
**Deliverables**:
- `.loomweave/guidance/` directory layout documented and parsed by `loomweave analyze`.
...
**Scope**:
- `loomweave serve` with two surfaces:
...
- Auth per ADR-012:
  ...
  - `serve.auth: none` is forbidden in production; if explicitly set, emit a
    `LMWV-SEC-SERVE-AUTH-NONE` finding at startup.
...
**Deliverables**:
- `loomweave serve` runs and accepts MCP connections; tool catalogue matches
...
### WP9 — Weft integrations (Filigree + Wardline, Loomweave side)
...
ADR-015 (Wardline→Filigree emission ownership; v0.1 = Loomweave-side SARIF translator),
...
- **Suite-compat probe**: at startup, `loomweave analyze` checks Filigree and Wardline
...
**Out of scope (explicit)**:
- Filigree `registry_backend: loomweave` mode (that's WP10 — Filigree-side work).
- Wardline-native finding emitter (deferred to v0.2 per ADR-015).
- Observation HTTP transport (deferred to v0.2 per ADR-016).

**Deliverables**:
- End-to-end test: `loomweave analyze` against a Python fixture writes findings both
...
- Filigree-side integration depends on WP10 having shipped `registry_backend: loomweave`
  *only if* Loomweave is operating in non-shadow mode against a real Filigree. In
...
- Severity-map round-trip: every Loomweave `LMWV-*` rule-ID has a defined Filigree-side
  severity; round-trip preserves the rule-ID.

**ADR triggers**:
- **ADR-020** (degraded-mode policy and explicit suite fallbacks, backlog) — the
  compat-probe implementation will surface the precise contract for "what does Loomweave
...
- **Filigree side** (separate repo, same author):
  ...
  - Define `RegistryProtocol` trait/interface; refactor file-registry call sites
    to go through it.
  - Implement `registry_backend: loomweave` mode that delegates to Loomweave's entity
  ...
  - Add `scan_source` field to Filigree's finding records so Loomweave-emitted findings
    can be distinguished from Filigree-native ones.
  - Contract test: a recorded Loomweave `findings.jsonl` POSTed to Filigree round-trips
    cleanly.
- **Loomweave side**:
  - `loomweave sarif import` translator that reads a Wardline SARIF file, translates
    to Loomweave's finding shape, and POSTs to Filigree (the v0.1 stand-in for the
...
**Deliverables**:
- Filigree PR (or commit series) implementing `RegistryProtocol` + the `loomweave`
  backend; Filigree's existing test suite continues to pass.
- `loomweave sarif import` subcommand with a Wardline SARIF fixture and a round-trip
  test.
...
- Filigree-side `registry_backend: loomweave` mode must ship before WP9's non-shadow
  mode is testable end-to-end.

**Exit criteria**:
- Filigree contract test passes against a Loomweave-emitted `findings.jsonl`.
- `loomweave sarif import` round-trip preserves rule-ID, severity, and the property-bag
...
**Anchoring docs**: [`../loomweave/v0.1/plans/v0.1-scope-commitments.md`](./v0.1-scope-plans/v0.1-scope-commitments.md)
§"Validation".

**Accepted ADRs**: none — this is a measurement task, not a design task.

**Scope**:
- Run `loomweave analyze` against `elspeth-slice` (smallest viable subset; target ~50
...
- Updated NFR rationale sections in [`../loomweave/v0.1/requirements.md`](../loomweave/v0.1/requirements.md)
  reflecting reality, not speculation.

**Sequencing constraints**:
- Requires WP6 (LLM dispatch) at minimum. WP9 + WP10 are not strictly required (the
  spike measures Loomweave-only metrics) but would be present if WP11 is the last
  package.
...
|---|---|---|---|
| ADR-005 | `.loomweave/` git-committable subpaths | WP1 | When implementing `loomweave install`, decide which subpaths are tracked vs ignored. |
...
| ADR-019 | SARIF property-bag preservation | WP10 | When the `loomweave sarif import` translator's property-mapping table is concrete. |
...
- **Q1**: Are the Filigree-side `registry_backend: loomweave` mode and the
...
- **Q3**: Should `loomweave analyze --no-llm` be a permanent supported mode (used by
...
- [Loomweave v0.1 requirements](../loomweave/v0.1/requirements.md)
- [Loomweave v0.1 system design](../loomweave/v0.1/system-design.md)
- [Loomweave v0.1 detailed design](../loomweave/v0.1/detailed-design.md)
- [Loomweave ADR index](../loomweave/adr/README.md)
- [v0.1 scope commitments memo](./v0.1-scope-plans/v0.1-scope-commitments.md)
- [Weft suite doctrine](../suite/weft.md)
