...
Loomweave no longer owns live GitHub branch/ruleset release-control enforcement.
...
secret before build jobs could start. That requirement is retired for Loomweave.
Governance enforcement is moving to Legis, which is delivered alongside the
next Loomweave release.

Loomweave's standalone release semantics are now:

- the release workflow verifies the source tree, builds artifacts, and publishes
  GitHub Releases for `v*` tags;
- Loomweave does not require live repository ruleset inspection to build or
  publish its own artifacts;
- any Legis governance signal is external enrichment and must not become a
  prerequisite for Loomweave to remain useful alone.

Keep future Loomweave release controls local to Loomweave's own artifact integrity
...
without adding a mandatory sibling dependency to Loomweave's release workflow.

## Current Release Sequence

1. Confirm the release branch or PR is green under Loomweave's CI floor.
