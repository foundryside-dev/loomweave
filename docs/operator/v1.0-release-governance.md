...
The release workflow uses this secret only in the `GitHub release governance`
job, after the untrusted triggering ref has passed the `verify` job. That job
checks out the governance guard from the reviewed `main` branch before exposing
the token, and the build jobs do not start unless the guard passes.
...
6. Run the release workflow manually from `main` before creating the tag:
   ...
   publish a GitHub Release. Run it from `main` only. It also runs the same
   governance guard with `RELEASE_GOVERNANCE_TOKEN` from the reviewed `main`
   checkout; do not proceed if that job fails.
