...
- `impl` blocks get a synthesized, source-order-independent discriminator appended to the target type:
  ...
  - **Inherent impl:** `…<Type>.impl#<positional-generic-signature>#<ordinal>`, with a **positional, De Bruijn-style** rendering of generic parameters (each param rendered as `$0`, `$1`, … so a param *rename* does not churn: `impl<T> Foo<T>` and `impl<U> Foo<U>` both render `impl#<$0>#0`; a non-generic `impl Foo` renders `impl#<>#0`). The trailing **ordinal** (`#0`, `#1`, …) is always present and assigned by source order *within a file*, so multiple inherent impls with an identical generic signature stay distinct, and — because two inherent blocks for the same type in different files is legal — they are disambiguated further by the defining file's module path already present in the prefix.
