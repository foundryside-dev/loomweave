...
| [ADR-049](./ADR-049-rust-qualname-canonicalization.md) | Rust plugin qualname canonicalization (the SEI *locator*) — dot-separated, crate-rooted, impl/trait-discriminated path that is unique-within-a-run and stable across benign edits (closes cross-crate + intra-type collisions and the generic-param-rename churn); positional De Bruijn generics, `@cfg` twin discriminant (impl blocks, free items, and impl-member methods), inherent-impl ordinal; uniqueness is a tested invariant, not the writer's silent `ON CONFLICT` merge; also records the syntactic (`syn`, not rust-analyzer) backend choice (network/credential posture). Relates to ADR-003/022/028/038 | Accepted; amended (ordinal-drop / self-type-args / generic-arg-rendering / cfg-twin-methods) |
