...
costs close to zero; retrofitting after any Loomweave/Wardline-scale code has
been written is expensive.

## Summary

The Loomweave workspace adopts a strict tooling baseline from its first code
...
The Loomweave repository sat at zero lines of code on 2026-04-18. Sprint 1's
...
2. **Workspace-level `[lints]` with pedantic enabled is near-free at
   ...
   floor from day one. The scope-commitment memo already commits Loomweave to
...
- Every future Loomweave commit passes pedantic clippy, rustfmt, cargo-deny,
...
- `cargo nextest` halves test-suite wall-clock time on workspace builds
  ...
  tests in `loomweave-storage` alone), the compounded savings are non-trivial.
