# ADR-037: Shared Error Vocabulary (`loomweave-core::errors`)
...
Co-locate two **separate** typed enums — `HttpErrorCode` and `McpErrorCode` — in a new `loomweave-core::errors` module as the single source of truth for Loomweave's two structured wire error surfaces. Each enum preserves its established wire spelling (SCREAMING_SNAKE for HTTP, kebab-case for MCP). Wire bytes are unchanged on both surfaces. Drift tests pin every wire string at the definition site.

## Context

Loomweave emits machine-routable error codes on two independent wire surfaces:
...
2. **MCP tool-error envelope** — consumed by consult-mode agents. Prior to this work: ~47 bare string literals scattered across `loomweave-mcp`, no type, no exhaustiveness checking, no compiler protection against typos, and no pinning of the wire strings in tests.
...
Add a `loomweave-core::errors` module containing two separate typed enums:
...
- **Single typed home for both vocabularies.** All additions, renames, and retirements require a one-line change in `loomweave-core::errors` plus a test assertion, not a grep through `loomweave-mcp`'s tool handlers.
...
- **`loomweave-cli` and `loomweave-mcp` now depend on `loomweave-core`** for their error code types. This is a crate-internal dependency increase, not a federation coupling; `loomweave-core` is already the shared crate for entity-ID logic, plugin host, and manifest parsing. The coupling cost is minimal.
...
- Module: [`crates/loomweave-core/src/errors.rs`](../../../crates/loomweave-core/src/errors.rs)
...
- Implementing commits:
  - `5afbbe0` feat(errors): add `loomweave-core::errors` with `HttpErrorCode` + `McpErrorCode`
