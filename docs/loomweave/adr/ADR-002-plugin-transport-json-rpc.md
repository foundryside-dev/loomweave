...
Loomweave plugins will run as subprocesses and speak JSON-RPC 2.0 over a Content-Length framed stream. This keeps plugin authorship language-specific while preserving resumable, binary-safe transport semantics.

## Context

The Loomweave core must communicate with language plugins that:
