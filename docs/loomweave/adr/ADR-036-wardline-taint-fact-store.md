...
**Amendment (2026-06-11, plugin-aware candidate minting)** (`clarion-69db8b2739`): §3 and the §Consequences route inventory below say resolution builds the candidate entity ID `python:function:<qualname>` — hardcoding the `python` plugin. With the first-party Rust language plugin shipped (1.x), an existing `rust:function:<qualname>` was therefore unresolvable through `/api/wardline/resolve`, the taint-fact read/write routes, and the `entity_resolve` MCP tool. **Decision:** the candidate plugins are the plugins that actually own function entities — `SELECT DISTINCT plugin_id FROM entities WHERE kind = 'function'`, queried once per batch call — and the resolver mints `{plugin}:function:<qualname>` for each. The qualname is **not** parsed to guess its plugin (qualnames are opaque per ADR-003/ADR-049); resolution still goes through the existing PK `IN`-probe (probe set = qualnames × plugins). A qualname that exists under exactly one plugin resolves `Exact` as before; one that exists under **more than one** plugin resolves a new `Resolution::Ambiguous { entity_ids }` variant (ids deterministically sorted). The `Resolution::{entity_id, into_entity_id}` accessors return `None` for `Ambiguous`, so every federation surface that drives off those accessors **degrades an ambiguous match to "unresolved"**: it is never written as a taint fact and never collapsed onto an arbitrarily-chosen plugin — preserving this ADR's exact-only-write contract and the single-id `ResolveResponse` wire shape Wardline consumes (no wire-shape change). The `entity_resolve` MCP tool, whose output was multi-candidate-shaped from day one, surfaces an ambiguous hit as `result_kind: "ambiguous"` listing every candidate (each still projected through `entity_json` for SEI attach + briefing-blocked stub collapse). One resolver definition; no normalization, no new migration or index. One consequence is **fact eclipse**: a fact persisted under `python:function:<q>` while the qualname was unambiguous becomes unreachable *by qualname* — though still reachable by SEI — if a same-qualname `rust:function:<q>` entity later appears, because the qualname now resolves `Ambiguous` and ambiguous degrades to unresolved on the federation wire (the stored row is untouched; reads simply report `exists: false`). Note also that the wire cannot distinguish ambiguous from not-found — both surface as `unresolved` — so a plugin-hint field on `ResolveRequest` is the eventual disambiguator, deferred to the Wardline rc5 handoff.

**Amendment (2026-06-11, `ResolveRequest` plugin hint)** (`clarion-b1a158f7f5`): the disambiguator deferred by the previous amendment is now landed, with the shape agreed consumer-side in Wardline's `docs/integration/2026-06-11-wardline-resolve-plugin-hint-proposal.md` (§7 of the Amendments 6–9 changeset letter). **Shape:** `POST /api/wardline/resolve` gains an OPTIONAL `plugin: string` field — an ADR-049 plugin id exactly as it appears in entity ids (`python`, `rust`), **batch-scoped** (one hint per request, not per qualname: every Wardline resolve batch is minted by exactly one frontend; a consumer with mixed-language qualnames sends one request per plugin). **Semantics:** the hint is a **constraint, never a preference order**. With `plugin: p`, resolution is restricted to that plugin's namespace — the `DISTINCT plugin_id` enumeration is skipped and exactly one candidate, `{p}:function:<qualname>`, is minted per qualname (`loomweave_storage::resolve_wardline_qualnames_for_plugin`); a unique match resolves, no match is `unresolved` **even when another plugin owns the qualname**. Without the hint, behavior is byte-for-byte the previous amendment's cross-plugin resolution (`Ambiguous` degrades to `unresolved` on the wire); omission stays legal forever. **Adjudicated edge cases** (the proposal left these Loomweave's call): a present-but-blank/whitespace-only `plugin` is a 400 whose message names the field — the proposal's rollout note asks rejections to name the field for cross-version diagnosability; any other non-blank string is honored as a constraint, so an **unknown plugin id is a 200 with the whole batch unresolved** — plugin ids are deliberately not validated against the store (no store-coupled vocabulary check; a hint for a plugin with zero entities is the same "constraint nothing satisfies" as a typo). No change to the `resolved`/`unresolved` response shape, no `ontology_version` interaction, no MCP-surface change (`entity_resolve` already returns multi-candidate output and needs no hint). **Ordering:** `ResolveRequest` is `#[serde(deny_unknown_fields)]`, so Loomweave lands first by construction; Wardline's client slice (threading the finding's `lang` through `LoomweaveClient.resolve()`, fail-soft on 4xx from an old server) is tracked as `wardline-7eccab28ee`. The proposal's three conformance rows (hinted-hit / hinted-miss / unhinted-ambiguous) are pinned as resolver + route tests until a hint-aware fixture section is agreed with that slice (see the TODO in `docs/federation/fixtures/wardline-qualname-normalization.json`).
