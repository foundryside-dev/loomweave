...
**Relates to**: [ADR-005](./ADR-005-loomweave-dir-tracking.md) (git-committable `.loomweave/` — this ADR **extends** its gitignore list by its own authority; ADR-005 is unchanged), [ADR-007](./ADR-007-summary-cache-key.md) (content-hash cache invalidation — mirrored), [ADR-030](./ADR-030-on-demand-summary-scope.md) (on-demand / policy-engine cost posture), [ADR-039](./ADR-039-llm-provider-pivot-openrouter-cli.md) (the `LlmProvider` abstraction this mirrors).
**Glossary verdict**: **no clash** — `search_semantic`, `EmbeddingProvider`, and `entity_embeddings` are Loomweave-local; no sibling product uses these terms. No `glossary.md` change required.
...
**Positive**
- Local-first preserved: off by default; honest degrade; no hosted service required for any other Loomweave semantics (enrich-only on the product's own surface).
- Committed `.loomweave/loomweave.db` stays unbloated; embeddings are rebuildable and never committed.
