...
A write that times out **after** delivering some bytes is different: pyright has consumed a `Content-Length: N` header and part of an N-byte body, and would splice the next file's `didOpen` onto that half-message. Every later request would then read-time-out until the wedge breaker respawned three files later, pinning three innocent files with sticky self-inflicted `pyright_timeout` marks (§4) for a corruption they did not cause. A partial write therefore **invalidates the transport**: the process is killed and its handle dropped, and the `LspTimeoutError` is re-raised so the overrunning file keeps its own honest self-inflicted mark. This is transport invalidation, not a restart policy — nothing is charged to `MAX_PYRIGHT_RESTARTS_PER_RUN`, no finding is emitted, and because the handle is dropped the next `_ensure_process` takes its "no process" branch and spawns silently, so the next file is never dead-on-arrival and never marked collateral `pyright_restarting`. The one-shot `restart_already_charged_to_file` flag (§3) is armed against the in-flight path, so the same file's remaining facet still respects the watchdog ceiling before respawning and the next file's spawn consumes it silently. Only `didOpen` can reach this state: every other message the plugin sends is below `PIPE_BUF` and therefore written atomically.

**Interpreter-unpinned is not a restart-attribution question ([ADR-058](./ADR-058-project-interpreter-discovery.md), clarion-5cf9643de9).** A run where discovery could not find a project-owned interpreter — no override, `.venv`, `VIRTUAL_ENV`, or `CONDA_PREFIX` — demotes an otherwise-`complete` calls/references facet to `degraded` with reason `interpreter_unpinned`, `transient=false`, `collateral=false`. This is environment-determined, exactly like `syntax_error` / `reference_site_cap` in §1 above: no catch-site in the pyright process ever sees a crash, timeout, or death, so it never enters the restart/attribution machinery this ADR governs. It only ever demotes a facet that would otherwise read `complete`; a facet already `degraded` for a real pyright-health reason keeps that reason. Recovery is not a restart or a re-dispatch-budget spend — it is the `plugin_index_meta.resolver_environment` fingerprint changing (an operator pins a `.venv` or sets `LOOMWEAVE_PYTHON_INTERPRETER`), which forces a full re-dispatch of the plugin's files independent of any `redispatch_attempts` counter.

`close()`'s teardown shares **one** deadline across the `shutdown` request and the `exit` notification (`PYRIGHT_SHUTDOWN_TIMEOUT_SECS` total, not per message). The `MAX_FILES_PER_PYRIGHT_SESSION` recycle runs inside an `analyze_file` call, so a per-message grant would let a wedged server spend twice that budget out of the terminal safety margin: worst case is the file's own ceiling (anchor + 105 s) + 5 s teardown + ≤ 2 s stderr join ≈ 112 s against the host's 120 s watchdog.
