...
**Context**: `loomweave analyze --resume RUN_ID` shipped as a run-lifecycle and
finding-emission repair path, while older design text still promised
phase/file checkpoint recovery.

## Summary

Loomweave's v1.x `--resume RUN_ID` reopens the existing `runs` row, re-walks the
...
contract remains: an unclean shutdown must not corrupt `.loomweave/loomweave.db`,
...
flapping, but Loomweave does not guarantee it will skip already completed
...
- Operators must not rely on `.loomweave/runs/<run_id>/partial.json` or
