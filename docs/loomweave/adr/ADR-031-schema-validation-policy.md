...
1. **Layer A — Writer-actor typed API (primary).** The writer-actor's
   ...
   structured `StorageError::WriterProtocol` errors with `LMWV-INFRA-*`
...
The schema-apply test suite (`crates/loomweave-storage/tests/schema_apply.rs`)
...
The writer-actor tests (`crates/loomweave-storage/tests/writer_actor.rs`)
...
operator pulls a published Loomweave build and produces a
`.loomweave/loomweave.db`. After that point, vocabulary expansions for any
...
at the `analyze_file` notification boundary (`LMWV-INFRA-PLUGIN-UNDECLARED-KIND`
...
- [Skeleton audit](../../implementation/handoffs/2026-05-03-skeleton-audit.md)
  ...
  motivated this ADR.
- `crates/loomweave-storage/migrations/0001_initial_schema.sql:80-81, 134`
  — the two existing CHECK precedents this ADR generalises.
- `crates/loomweave-storage/src/commands.rs:26-43` — `RunStatus` Rust enum;
  the typed Layer-A validator for `runs.status` writes.
- `crates/loomweave-storage/src/writer.rs:322` — the `'running'` hand-typed
  literal at `BeginRun`; the kind of fat-finger the CHECK catches.
- `crates/loomweave-storage/src/writer.rs:411` — `enforce_edge_contract`,
  the Layer-A validator for `edges` writes.
- `crates/loomweave-storage/tests/schema_apply.rs:538` —
