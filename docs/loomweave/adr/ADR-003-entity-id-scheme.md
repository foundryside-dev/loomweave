...
**Context**: stable cross-tool identity for Loomweave entities

## Summary

Loomweave entity IDs will use symbolic canonical qualified names rather than file-path-embedded identifiers. File paths remain properties, while rename tracking beyond the 80% case is deferred to a later `EntityAlias` mechanism.

## Context

Loomweave's entity IDs are referenced by:

- the Loomweave catalog itself
...
- identity translation stays with Loomweave rather than leaking into sibling tools
