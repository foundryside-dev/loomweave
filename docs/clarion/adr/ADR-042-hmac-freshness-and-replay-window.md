...
Clarion's protected HTTP routes keep ADR-034's preferred Loom component HMAC
mode, but every signed request now carries `X-Loom-Timestamp` and
`X-Loom-Nonce`. The HMAC canonical message is:
...
Clarion accepts timestamps inside a five-minute skew window and rejects reuse of
...
- Require `X-Loom-Timestamp` as Unix seconds and `X-Loom-Nonce` as a non-empty
