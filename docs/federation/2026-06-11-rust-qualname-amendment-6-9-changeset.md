...
## 7. Riding the same handoff: `ResolveRequest` plugin-hint (LANDED Loomweave-side)

Loomweave's qualname resolver is now plugin-aware (ADR-036, amended 2026-06-11, `clarion-69db8b2739`): a qualname owned by more than one plugin resolves `Ambiguous`, and **the federation wire degrades ambiguous to `unresolved`** — indistinguishable from not-found in the `ResolveResponse` you consume. The disambiguator is a **plugin-hint field on `ResolveRequest`**. That struct is `#[serde(deny_unknown_fields)]` on the Loomweave side, so Wardline cannot simply start sending the field — adding it was a **deliberate, coordinated contract change** to the resolve wire shape.

**Status: landed Loomweave-side (this commit, `clarion-b1a158f7f5`).** The shape is exactly as agreed in Wardline's proposal doc (`wardline/docs/integration/2026-06-11-wardline-resolve-plugin-hint-proposal.md`): OPTIONAL batch-scoped `plugin: string` (ADR-049 plugin id); the hint is a constraint, never a preference order — unique match in the named plugin → resolved, no match → `unresolved` even when another plugin owns the qualname; omission stays legal forever and is byte-for-byte the unhinted behavior. The choice the proposal left open is adjudicated as: blank/whitespace-only `plugin` → 400 naming the field; any other non-blank value → constraint semantics (unknown plugin id = 200, whole batch unresolved). Normative semantics live in ADR-036's 2026-06-11 plugin-hint amendment paragraph. **The Wardline client slice** (thread the finding's `lang` through `LoomweaveClient.resolve()` at both call sites, fail-soft `unresolved` on a 4xx from an old server) **is tracked as `wardline-7eccab28ee`**; the proposal's three conformance rows ride that slice (pinned Loomweave-side as resolver + route tests meanwhile — see the TODO in `docs/federation/fixtures/wardline-qualname-normalization.json`).
