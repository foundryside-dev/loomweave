...
**Authority:** Loom SEI conformance standard §7 / §7.1; Clarion ADR-038.

---

## What this migration does

The Loom suite is moving every cross-tool binding off the mutable **locator**
(`{plugin}:{kind}:{qualname}`) and onto the durable, opaque **SEI**
(`clarion:eid:<hex>`). Before the cutover, producers store and emit locators;
...
|---|---|
| **Clarion** (authority) | Mints an SEI for **every** current entity on its first SEI-aware analyze run, and serves `resolve(locator)` so producers can re-key. This is the only Clarion action — it has no cross-tool bindings of its own to re-key. |
| **Filigree** | Re-keys every stored `clarion_entity_id` association from its locator to the resolved SEI. |
...
## Clarion's side (already shipped in Wave 1)
...
1. **Mint-all-on-first-run.** The analyze SEI mint pass (ADR-038 §3) mints an
   ...
   separate "backfill job" is needed on Clarion's side — minting *is* the
...
3. **Reserved-prefix rejection (REQ-F-02) — the safety interlock.** `resolve`
   **rejects** any SEI-shaped input (reserved `clarion:eid:` prefix) with `400`.
...
side table). No Clarion-side generation marker is required.

For each stored binding the producer holds:

1. Read the stored id. If it already begins with `clarion:eid:`, it is **already
...
2. **Clarion cuts first.** Deploy SEI-aware Clarion and run analyze so every
...
4. **Flip the feeds.** Every federation feed that carries entity ids
   (e.g. Filigree's `affected_entities` on `GET /api/loom/changes`) switches to
...
then Clarion mints and serves SEIs additively (enrich-only); consumers that have
