...
/// An anchored `references` edge from the mentioning entity (`from_id` — the
/// struct/enum whose field names the type, the function whose signature/body
/// mentions the path, the `type_alias`/`const`/`static` whose RHS does) to the
/// resolved target (`to_id`), carrying the PATH TOKEN's byte span (NOT the
/// enclosing item or expression).
///
/// Like its siblings, `references` is anchored (ADR-026 decision 3): it
...
/// target, `"ambiguous"` for a multi-kind candidate. An `External` (or
/// unmatched) path yields NO edge (dropped + counted, D4), so this helper is
/// never called for it.
#[must_use]
pub fn references_edge(from_id: &str, to_id: &str, confidence: &str, span: &SourceRange) -> Value {
    json!({
        "kind": "references",
        "from_id": from_id,
        "to_id": to_id,
