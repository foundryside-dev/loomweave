//! Git-ignored embeddings sidecar (`.clarion/embeddings.db`) for `WS5b` semantic
//! search (ADR-040).
//!
//! Embeddings are large and rebuildable, so they must **not** bloat the
//! committed `.clarion/clarion.db` (ADR-005). They live in a separate `SQLite`
...
/// The conventional sidecar path for a project: `<root>/.clarion/embeddings.db`.
#[must_use]
pub fn embeddings_db_path(project_root: &Path) -> PathBuf {
    project_root.join(".clarion").join("embeddings.db")
}
...
impl EmbeddingStore {
    ...
    /// Open the conventional `<root>/.clarion/embeddings.db` sidecar.
    pub fn open_in_clarion_dir(project_root: &Path) -> Result<Self> {
