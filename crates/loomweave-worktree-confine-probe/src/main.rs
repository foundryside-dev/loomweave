...
//!
//! **Platform scope: Linux-only.** `unshare`/`UnshareFlags` and the mount
//! APIs this binary needs (`rustix::thread`'s `setns` module,
//! `rustix::mount`) are themselves gated by `rustix` to `cfg(linux_kernel)`
//! and do not exist on e.g. `aarch64-apple-darwin` — there is no portable
//! namespace/mount primitive to fall back to, unlike
//! [`loomweave_cli::worktree::confine`], which has a real cross-platform
//! "unsupported" outcome. So the entire privileged sequence lives in
//! [`linux`], and `main` on any other platform just reports that and exits
//! nonzero — there is nothing for this probe to usefully do off Linux, since
//! its only job is to drive a Linux-only bind-mount scenario.
...
mod linux {
    ...
    /// because `UnshareFlags::FILES` can leave threads observing
    /// inconsistent file-descriptor tables. This binary passes only
    /// `NEWUSER | NEWNS`, neither of which touches file-descriptor-table
    /// sharing, and — being a single-threaded `fn main()` with no other
    /// threads ever spawned — has no other thread that could observe such
    /// an inconsistency regardless. Using the deprecated *safe* wrapper
    /// here, rather than `unshare_unsafe`, keeps this workspace's
    /// `unsafe_code = "deny"` policy at its single documented exception
    /// (`plugin/host.rs`'s `pre_exec`).
    #[allow(deprecated)]
    fn unshare_user_and_mount_namespace(flags: UnshareFlags) -> Result<(), Errno> {
        rustix::thread::unshare(flags)
    }

    /// Every step of acquiring and configuring the namespace, folded into
    /// one fallible sequence: `unshare` itself, the three `/proc/self/*`
    /// writes, and the final `mount_change`. On some runners the *first*
    /// failure isn't `unshare()` — see the module docs — so every step
    /// here is reported identically, tagged with which one failed.
    fn setup_namespace() -> Result<(), (&'static str, String)> {
        ...
        // Make this mount namespace's root private+recursive before
        // mounting anything, so the bind mount below never propagates back
    ...
    /// Same classification as [`classify_errno`], for the `std::io::Error`
    /// the `/proc/self/*` writes return (`fs::write`, not a raw `rustix`
    /// call). `EACCES` is folded into the same `"EPERM"` tag as `EPERM`
    /// itself: both mean "this process lacks a capability it needs," which
    /// is exactly the distinction the caller's diagnostic cares about —
    /// not which specific errno the kernel or an LSM (e.g. `AppArmor`'s
    /// unprivileged-userns restriction) happened to return.
    ...
    /// The real, Linux-only body of `main`.
...
fn main() -> ExitCode {
    ...
    {
        eprintln!(
            "loomweave-worktree-confine-probe: unsupported platform — this probe drives \
             Linux-only namespace/mount syscalls (unshare, bind mounts) it has no portable \
             fallback for, unlike the primitive it tests \
             (loomweave_cli::worktree::confine::WorktreesRoot). Nothing to do off Linux."
        );
        ExitCode::FAILURE
    }
}
