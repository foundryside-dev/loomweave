//! SEI conformance oracle (Weft SEI standard §8) — the shared, fixtures-based
//! conformance suite, exercised against a reference Loomweave (this crate's real
...
fn apply_run(
    ...
    for d in ordered {
        ...
        let (sei, event, is_carry) = match decision {
            loomweave_storage::SeiDecision::Carry { sei, event } => (sei, event, true),
            loomweave_storage::SeiDecision::Mint { sei } => (sei, Some(LineageEvent::Born), false),
        };
...
fn oracle_capability_absent_degrades_gracefully() {
    // A fresh DB before any SEI run models a pre-SEI / capability-absent Loomweave:
    ...
    match resolve_sei(&conn, "loomweave:eid:deadbeefdeadbeefdeadbeefdeadbeef").unwrap() {
