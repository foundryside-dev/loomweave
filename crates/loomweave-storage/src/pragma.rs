...
/// `SQLite` `application_id` header value identifying Loomweave databases.
///
/// ASCII "LMWV" — picked so `file(1)` / `sqlite3 .dbinfo` distinguishes a
/// Loomweave DB from any other `SQLite` file. Set lazily on first open of a
/// fresh (`application_id = 0`) file. Refusing to open a file with any
/// other non-zero value is how we close STO-02.
pub const LOOMWEAVE_APPLICATION_ID: u32 = 0x4C4D_5756;
...
/// [`LOOMWEAVE_APPLICATION_ID`] is rejected with
...
/// non-zero `application_id` that is not Loomweave's.
...
/// header. Accepts legacy zero-id files, accepts Loomweave's id, and rejects any
...
pub fn validate_application_id_for_read(conn: &Connection) -> Result<()> {
    ...
    match id {
        0 => Ok(()),
        id if id == LOOMWEAVE_APPLICATION_ID => Ok(()),
...
/// Read `application_id`; on `0` set it to [`LOOMWEAVE_APPLICATION_ID`]; on
/// [`LOOMWEAVE_APPLICATION_ID`] continue; on any other value refuse with
...
/// against [`LOOMWEAVE_APPLICATION_ID`] correctly.
    ...
    match current {
        0 => {
            conn.execute_batch(&format!(
                "PRAGMA application_id = {LOOMWEAVE_APPLICATION_ID};"
            ))?;
            Ok(())
        }
        id if id == LOOMWEAVE_APPLICATION_ID => Ok(()),
