...
//!
//! Implements ADR-038 and the Weft SEI conformance standard §3 (matcher),
...
//! persisted `sei_bindings`. Loomweave's byte-identical-run guarantee covers
...
/// does.
pub const SEI_PREFIX: &str = "loomweave:eid:";
...
///
/// `loomweave:eid:<lowercase-hex(blake3(utf8(locator) ++ 0x00 ++ utf8(mint_run_id)))[:32]>`.
///
...
/// never "Loomweave's git code" (REQ-C-05) — so `legis` can supply a concrete
...
/// (`ShellGitRenameSource`) lives in `loomweave-cli`, where the qualname/path
...
/// nothing alive.
///
/// Callers MUST reject reserved (`loomweave:eid:`-prefixed) inputs *before*
...
mod tests {
    ...
    fn matcher_carries_unchanged_locator_with_no_event() {
        ...
        assert_eq!(
            decision,
            SeiDecision::Carry {
                sei: "loomweave:eid:0001".to_owned(),
                event: None,
            }
        );
    }
    ...
    fn matcher_carries_move_on_identical_body_and_signature() {
        ...
        let alive = HashMap::from([(
            "a.mod.f".to_owned(),
            binding("loomweave:eid:0001", "a.mod.f", "h1", Some("s1")),
        )]);
    ...
    fn matcher_fails_closed_when_move_is_ambiguous() {
        ...
        let alive = HashMap::from([
            ...
            (
                "a.g".to_owned(),
                binding("loomweave:eid:0002", "a.g", "h1", Some("s1")),
            ),
        ]);
    ...
    fn matcher_does_not_move_match_on_null_signature() {
        ...
        let alive = HashMap::from([(
            "a.f".to_owned(),
            binding("loomweave:eid:0001", "a.f", "h1", None),
        )]);
        let cur = locset(&["b.f"]);
    ...
    fn orphans_vanished_unmatched_bindings_only() {
        let alive = HashMap::from([
            (
                "still.here".to_owned(),
                binding("loomweave:eid:0001", "still.here", "h1", Some("s1")),
            ),
            (
                "renamed.old".to_owned(),
                binding("loomweave:eid:0002", "renamed.old", "h2", Some("s2")),
            ),
            (
                "deleted.gone".to_owned(),
                binding("loomweave:eid:0003", "deleted.gone", "h3", Some("s3")),
            ),
        ]);
    ...
    fn alive_snapshot_excludes_orphaned_and_keys_by_locator() {
        ...
        insert_binding(
            ...
            &SeiBinding {
                ...
                ..binding("loomweave:eid:0002", "a.g", "h2", Some("s2"))
            },
        );
        let snap = alive_bindings_snapshot(&conn).unwrap();
        assert_eq!(snap.len(), 1);
        assert_eq!(snap["a.f"].sei, "loomweave:eid:0001");
        assert_eq!(
    ...
    fn upsert_binding_carry_moves_locator_in_place_without_collision() {
        ...
        upsert_sei_binding(&conn, &record("loomweave:eid:0001", "auth.login", "r1")).unwrap();
        ...
        let carried = SeiBindingRecord {
            ...
            ..record("loomweave:eid:0001", "authn.login", "r1")
        };
        ...
        let born: String = conn
            .query_row(
                "SELECT born_run_id FROM sei_bindings WHERE sei = 'loomweave:eid:0001'",
                [],
                |r| r.get(0),
            )
            .unwrap();
    ...
    fn orphan_flips_status_and_frees_the_locator() {
        let conn = migrated_conn();
        upsert_sei_binding(&conn, &record("loomweave:eid:0001", "gone.f", "r1")).unwrap();
        orphan_sei_binding(&conn, "loomweave:eid:0001", "r2", "t1").unwrap();
        // resolve(locator) no longer finds it alive.
        assert!(resolve_locator(&conn, "gone.f").unwrap().is_none());
        // resolve_sei reports not-alive.
        assert!(matches!(
            resolve_sei(&conn, "loomweave:eid:0001").unwrap(),
            SeiLookupResult::NotAlive { .. }
        ));
        // A NEW binding may now take the freed locator (partial index excludes
        // the orphaned row).
        upsert_sei_binding(&conn, &record("loomweave:eid:0002", "gone.f", "r2")).unwrap();
        assert_eq!(
    ...
    fn lineage_appends_in_order_and_resolve_sei_returns_it_when_orphaned() {
        let conn = migrated_conn();
        upsert_sei_binding(&conn, &record("loomweave:eid:0001", "a.f", "r1")).unwrap();
        append_sei_lineage(
            &conn,
            &SeiLineageEntry {
                sei: "loomweave:eid:0001".to_owned(),
        ...
        let events: Vec<String> = sei_lineage(&conn, "loomweave:eid:0001")
            .unwrap()
            .into_iter()
            .map(|r| r.event)
            .collect();
        ...
        match resolve_sei(&conn, "loomweave:eid:0001").unwrap() {
