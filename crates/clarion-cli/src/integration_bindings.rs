...
fn clarion_yaml_ok(project_root: &Path, desired: &DesiredBindings) -> Result<bool> {
    let path = project_root.join("clarion.yaml");
    if !path.exists() {
        return Ok(false);
    }
        ...
        && value
            ...
            .is_some_and(|http| {
                http.get("enabled").and_then(Value::as_bool) == Some(true)
                    && http.get("bind").and_then(Value::as_str) == Some(CLARION_HTTP_BIND)
...
fn wardline_yaml_ok(project_root: &Path, desired: &DesiredBindings) -> Result<bool> {
    ...
    Ok(value
        .get("clarion")
        .and_then(|clarion| clarion.get("url"))
        .and_then(Value::as_str)
        == Some(CLARION_HTTP_URL)
        && value
            .get("filigree")
...
fn install_clarion_yaml(project_root: &Path, desired: &DesiredBindings) -> Result<bool> {
    let path = project_root.join("clarion.yaml");
