...
#   - `loomweave install` creates `.loomweave/loomweave.db`
#   - `loomweave analyze .` spawns the Python plugin and persists at least one entity
#   - `sqlite3 .loomweave/loomweave.db` returns Python module/function entities
...
loomweave install
[ -f "$DEMO_DIR/.loomweave/loomweave.db" ] || fail ".loomweave/loomweave.db not created by loomweave install"
...
INTEGRITY=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" "PRAGMA integrity_check;")
...
SOURCE_METADATA=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
...
if [ "$SOURCE_METADATA" != "$SOURCE_METADATA_EXPECTED" ]; then
    ...
    sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
...
FILE_ANCHOR=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
...
MODULE_PARENT=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
...
EDGE_RESULT=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
...
DROPPED=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
...
RESOLVED_CALLS=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
    "select count(*) from edges where kind = 'calls' and confidence = 'resolved';")
if [ "$RESOLVED_CALLS" -lt 1 ]; then
    log "DB edge contents:"
    sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" "select kind, from_id, to_id, confidence, properties from edges order by kind, from_id, to_id;" >&2 || true
...
AMBIGUOUS_WITH_CANDIDATES=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
...
AMBIGUOUS_EDGES=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
...
UNRESOLVED_CALL_SITES=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
...
RESOLVED_REFERENCES=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
...
REFERENCE_SITES=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
    "select json_extract(stats, '\$.reference_sites_total') from runs where status = 'completed';")
REFERENCES_RESOLVED=$(sqlite3 "$DEMO_DIR/.loomweave/loomweave.db" \
