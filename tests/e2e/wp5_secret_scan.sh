...
YAML
PATH="$PLUGIN_DIR" "$CLARION_BIN" analyze "$BASELINE_DIR"
BDB="$BASELINE_DIR/.clarion/clarion.db"
...
B_SECRET=$(sqlite3 "$BDB" "select count(*) from findings where rule_id = 'CLA-SEC-SECRET-DETECTED';")
[ "$B_SECRET" = "0" ] || fail "baseline-suppressed: expected 0 SECRET_DETECTED findings, got $B_SECRET"
B_BASELINE=$(sqlite3 "$BDB" "select count(*) from findings where rule_id = 'CLA-INFRA-SECRET-BASELINE-MATCH';")
...
OVERRIDE_DIR="$(mktemp -d -t clarion-wp5-override-XXXXXX)"
"$CLARION_BIN" install --path "$OVERRIDE_DIR"
printf "aws_access_key_id = 'AKIAIOSFODNN7EXAMPLE'\n" > "$OVERRIDE_DIR/leaky.sec"
PATH="$PLUGIN_DIR" "$CLARION_BIN" analyze \
    --allow-unredacted-secrets \
    --confirm-allow-unredacted-secrets=yes-i-understand \
    "$OVERRIDE_DIR"
ODB="$OVERRIDE_DIR/.clarion/clarion.db"
...
O_SECRET=$(sqlite3 "$ODB" "select count(*) from findings where rule_id = 'CLA-SEC-SECRET-DETECTED';")
[ "$O_SECRET" = "1" ] || fail "override is additive per ADR-013: expected 1 SECRET_DETECTED finding, got $O_SECRET"
O_OVERRIDE=$(sqlite3 "$ODB" "select count(*) from findings where rule_id = 'CLA-SEC-UNREDACTED-SECRETS-ALLOWED';")
...
MALFORMED_DIR="$(mktemp -d -t clarion-wp5-malformed-XXXXXX)"
"$CLARION_BIN" install --path "$MALFORMED_DIR"
printf "harmless = 'nothing'\n" > "$MALFORMED_DIR/clean.sec"
printf "not: valid: yaml: [\n" > "$MALFORMED_DIR/.clarion/secrets-baseline.yaml"
set +e
PATH="$PLUGIN_DIR" "$CLARION_BIN" analyze "$MALFORMED_DIR" 2>/dev/null
MALFORMED_EXIT=$?
set -e
[ "$MALFORMED_EXIT" -ne 0 ] || fail "malformed baseline: expected non-zero exit, got $MALFORMED_EXIT"
MDB="$MALFORMED_DIR/.clarion/clarion.db"
...
R_BASELINE_HITS=$(sqlite3 "$RDB" "select count(*) from findings where rule_id = 'CLA-INFRA-SECRET-BASELINE-MATCH';")
